...
function ActionItemRow({ item, onDeleted }) { // 항목 개수가 가변적이라 MeetingDetailPage가 아닌 자체 state로 관리, onDeleted로 삭제만 부모에 알림
  ...
  return ( // Actionitem 개별 항목 렌더링 
    <div>
      ...
      <button onClick={handleDelete} disabled={saving}>삭제</button>
      {error && <p>{error}</p>}
    </div>
  )
}
...
function MeetingDetailPage() {
  ...
  return (
    <>
      ...
      <button onClick={addDecision}>결정사항 추가</button>

      <h3>논의사항</h3>
      {discussions.map((d, i) => (
        <div key={i}>
          <input value={d} onChange={(e) => updateDiscussion(i, e.target.value)} />
          <button onClick={() => removeDiscussion(i)}>삭제</button>
        </div>
      ))}
      <button onClick={addDiscussion}>논의사항 추가</button>

      <button onClick={handleSaveMeeting} disabled={saving || !title}>
        {saving ? '저장 중...' : '회의록 저장'}
      </button>
      <button onClick={handleDeleteMeeting} disabled={saving}>회의록 삭제</button>
