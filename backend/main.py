...
router = APIRouter(prefix="/api") # React Router의 /meetings 페이지 경로와 FastAPI의 /meetings 리소스 경로 이름이 겹치기 때문에 ApiRouter에 Prefix를 붙혀 구별
                                  # 기존의 모든 기능들을 app이 아니라 APIRouter 인스턴스(객체)에 포함시키기 위함 
...
# 기존 api를 타던 함수들에 대해서 router로 변경함으로써 /api 주소임을 명시 


@router.post("/analyze", response_model=AnalyzeResponse) # FastAPI에서 /analyze 경로로 POST 요청이 들어오면 analyze 함수 실행 후 결과 반환
...
@router.post("/meetings", response_model=MeetingSaveResponse) # response_model로 지정한 형식에 맞춰서 meeting객체 리턴 ( id , title , created_at )
...
@router.get("/meetings", response_model=List[MeetingSaveResponse]) # 목록 조회: 저장 응답과 동일한 요약 형태(id, title, created_at) 재사용
...
@router.get("/meetings/{meeting_id}", response_model=MeetingDetail) # 상세 조회: action_items까지 포함한 전체 데이터 
...
@router.put("/meetings/{meeting_id}", response_model=MeetingDetail) # 특정 회의록 필드 전체 교체[수정] (action_items 목록 자체는 안 건드림)
...
@router.put("/action-items/{item_id}", response_model=ActionItemOut) # 액션아이템 필드 전체 교체[수정]
...
@router.post("/meetings/{meeting_id}/action-items", response_model=ActionItemOut) # 회의록 하나에 액션아이템 단일 추가 (배열 추가는 범위 밖)
...
@router.delete("/meetings/{meeting_id}", status_code=204)  # 삭제 성공 시 돌려줄 데이터가 없어기 때문에 204 No Content로 명시 (cascade로 action_items도 함께 삭제)
...
@router.delete("/action-items/{item_id}", status_code=204) # 삭제 성공 시 돌려줄 데이터가 없어서 204 No Content로 명시 (회의록 자체는 안 건드림)
